...
def main():
    ...
    parser.add_argument("--threshold", type=float, default=75.0, help="Skill match threshold %%")
    ...
    if args.search_only:
        search = JobSearch()
        import asyncio
        jobs = asyncio.run(search.search_jobs())
    ...
    applier = JobApplier(match_threshold=args.threshold, max_days_old=args.max_days, ui=ui, max_parallel=1)
    
    # Pass collector to UI for historical data loading
    if ui:
        ui.collector = applier.collector
        ui._load_current_run_data()
    ...
    # max_jobs_per_role is ignored - process all cards
    result = applier.run(max_jobs_per_role=20)
