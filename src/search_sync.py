        ...
        for keyword in keywords:
            with sync_playwright() as p:
                ...
                try:
                    ...
                    jobs = self._search_keyword(page, keyword, salary_min, salary_max, job_types)
    ...
    def _apply_filters(self, page: Any, salary_min: int, salary_max: int, job_types: list[str]) -> None:
        ...
        try:
            ...
            if salary_btn and salary_btn.is_visible():
                ...
                min_input = page.query_selector("input[placeholder*='Min'], input[name*='min' i]")
                max_input = page.query_selector("input[placeholder*='Max'], input[name*='max' i]")
                if min_input:
                    min_input.fill(str(salary_min // 100000))
                if max_input:
                    max_input.fill(str(salary_max // 100000))
    ...
    def _extract_jobs(self, page: Any) -> list[dict[str, Any]]:
        ...
        for card in cards:
            try:
                ...
                location_elem = card.query_selector(".location, [class*='location'], .locWdth, .location-span")
