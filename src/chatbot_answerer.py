#!/usr/bin/env python3
"""
Chatbot Answerer for Naukri Job Application (Async version)
...
class ChatbotAnswerer:
    ...
    async def answer_question(
        ...
        if await self._try_profile_qa(question, chatbot_container):
            return True
        ...
        radios = await chatbot_container.query_selector_all("input[type='radio']")
        is_yes_no = False
        yes_no_radios = []
        for radio in radios:
            if await radio.is_visible():
                label = await self._get_radio_label(radio, chatbot_container)
        ...
        if is_yes_no:
            ...
            return await self._fill_answer(chatbot_container, answer, question, page)
        ...
        if skill:
            if await self._try_skill_answer(skill, question, chatbot_container):
                return True
            # Unknown skill -> answer "0 years"
            if await self._fill_answer(chatbot_container, "0 years", question, page):
                self._debug(f"  [Chatbot] Unknown skill '{skill}', answered '0 years'")
                return True

        # 4. Try common patterns
        if await self._try_common_patterns(question, chatbot_container):
            return True

        # 5. Interactive mode - wait for user to answer
        if interactive:
            return await self._interactive_answer(question, chatbot_container, page)

        # 6. Could not answer
        self._debug(f"  [Chatbot] Could not answer: {question[:80]}")
        return False

    async def _interactive_answer(self, question: str, chatbot_container: ElementHandle, page: Page) -> bool:
        ...
        answer = await self._get_entered_answer(chatbot_container, page)
        if answer:
            # Save to profile Q&A for future use
            await self._save_answer_to_profile(question, answer)
            print(f"  [Chatbot] Saved answer: '{answer}' for future use")
            return True

        return False

    async def _get_entered_answer(self, chatbot_container: ElementHandle, page: Page) -> Optional[str]:
        """Try to extract the answer that was manually entered."""
        try:
            # Try text input
            text_input = await chatbot_container.query_selector("input[type='text'], textarea, input:not([type])")
            ...
            # Try dropdown
            select = await chatbot_container.query_selector("select")
    ...
    async def _save_answer_to_profile(self, question: str, answer: str) -> None:
    ...
    async def _try_profile_qa(self, question: str, chatbot_container: ElementHandle) -> bool:
            ...
            if stored_clean in q_lower or q_lower in stored_clean:
                ...
                if RAPIDFUZZ_AVAILABLE:
                    if fuzz.ratio(stored_clean, q_lower) > 80:
                        return await self._fill_answer(chatbot_container, answer, question, None)
    ...
    async def _try_common_patterns(self, question: str, chatbot_container: ElementHandle) -> bool:
    ...
    async def _fill_answer(self, chatbot_container: ElementHandle, answer: str, question: str, page: Page = None) -> bool:
        ...
        try:
            ...
            if select and await select.is_visible():
                ...
                for option in options:
                    if (await option.inner_text()).strip().lower() == answer.lower():
    ...
    async def _get_radio_label(self, radio: ElementHandle, container: ElementHandle) -> Optional[str]:
        ...
        try:
            ...
            # Try nearby text
            return (await radio.evaluate("el => el.nextElementSibling?.textContent || el.parentElement?.textContent || ''")).strip()
        except Exception:
            pass
        return None
